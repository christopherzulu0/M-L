                  ...
                  {item.submenu ? (
                    <DropdownMenu onOpenChange={(open) => setActiveDropdown(open ? item.name : null)}>
                      <DropdownMenuTrigger asChild>
                        <Button
                          variant="ghost"
                          className={`relative h-10 lg:h-12 px-2 lg:px-4 font-semibold transition-all duration-200 rounded-xl group text-sm lg:text-base ${pathname.startsWith(item.href)
                          ...
                          <DropdownMenuItem key={subItem.name} asChild className="p-0">
                            ...
                            >
                              ...
                              {subItem.badge && (
                                <Badge
                                  variant="secondary"
                                  className={`text-xs ${subItem.badge === "Hot" ? "bg-red-100 text-red-700" : "bg-blue-100 text-blue-700"
                                    }`}
                                >
                                  {subItem.badge}
                                </Badge>
                    ...
                    <Link
                      href={item.href}
                      className={`relative flex items-center space-x-1 lg:space-x-2 h-10 lg:h-12 px-2 lg:px-4 font-semibold rounded-xl transition-all duration-200 group text-sm lg:text-base ${pathname === item.href
                          ? "text-blue-600 bg-blue-50"
                          : "text-slate-700 hover:text-blue-600 hover:bg-blue-50"
                        }`}
                    >
                      ...
                      <div className="max-h-80 overflow-y-auto">
                        {loadingNotifications ? (
                          ...
                          notifications.map((notification) => (
                            <div
                              key={notification.id}
                              className={`p-4 hover:bg-slate-50 transition-colors border-b border-slate-100 last:border-0 ${!notification.read ? 'bg-blue-50/50' : ''
                                }`}
                            >
                              <div className="flex items-start space-x-3">
                                <div
                                  className={`w-8 h-8 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center ${notification.type === "listing"
                                      ? "bg-green-100"
                ...
                <div className="space-y-2">
                  {navigationLinks.map((item) => (
                    <div key={item.name} className="space-y-2">
                      <Link
                        href={item.href}
                        className={`flex items-center justify-between p-4 rounded-xl font-semibold transition-all duration-200 ${pathname === item.href || pathname.startsWith(item.href)
                ...
                {isSignedIn ? (
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <Link href="/sign-in" className="w-full">
                      <Button variant="outline" className="w-full h-10 rounded-xl border-slate-300">
                        Sign In
                      </Button>
                    </Link>
